...
#[derive(Parser)]
#[command(name = "infigraph", version, about = "AST-powered code analysis and impact review")]
struct Cli {
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Search { query, limit, alpha } => cmd_search(&root, &query, limit, alpha),
        ...
        Commands::SearchCode { pattern, file_pattern, limit } => {
        ...
        Commands::VisualizeSymbol { symbol_id, depth } => cmd_visualize_symbol(&root, &symbol_id, depth),
        ...
        Commands::Security { severity, category } => cmd_security(&root, severity.as_deref(), category.as_deref()),
        Commands::Complexity { threshold, file } => cmd_complexity(&root, threshold, file.as_deref()),
        ...
        Commands::Refactor { target, focus, limit } => cmd_refactor(&root, target.as_deref(), &focus, limit),
    }
}

fn cmd_init(root: &PathBuf, group: Option<&str>) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_index(root: &PathBuf, full: bool, no_embed: bool) -> Result<()> {
    if full {
    ...
    println!("Indexed {}/{} files", result.indexed_files, result.total_files);
    ...
    if no_embed { auto_scip(root, &result)?; return Ok(()); }
    {
...
fn try_install_lsp(lsp_server: &str) -> bool {
    if on_path(lsp_server) { return true; }
    ...
    let installs: &[(&str, &[(&str, &str, &[&str])])] = &[
        ...
        ("lua-language-server", &[
            ...
            ("linux",  "apt-get", &["install", "-y", "lua-language-server"]),
        ]),
        ("clangd", &[
        ...
        ("fsautocomplete", &[
            ("any", "dotnet", &["tool", "install", "-g", "fsautocomplete"]),
        ]),
        ("pasls", &[
        ...
        ("pls", &[
            ("any", "cpan", &["App::PerlLanguageServer"]),
        ]),
    ];

    let avail = |installer: &str| match installer {
        "npm"    => has_npm,   "pip3" | "pip" => has_pip,
        "gem"    => has_gem,   "cargo"  => has_cargo,
        "brew"   => has_brew,  "apt-get" => has_apt,
        "opam"   => has_opam,  "ghcup"  => has_ghcup,
        "dotnet" => has_dotnet,"rustup" => on_path("rustup"),
        _ => false,
    };

    if let Some((_, cmds)) = installs.iter().find(|(s, _)| *s == lsp_server) {
        for (target_os, installer, args) in *cmds {
            if (*target_os != "any" && *target_os != os) || !avail(installer) { continue; }
...
fn run_scip_indexer(root: &PathBuf, cmd: &str, args: &[&str], label: &str) -> bool {
    ...
    match std::process::Command::new(cmd).args(args).current_dir(root).status() {
        ...
        Ok(s) => { eprintln!("Auto-SCIP: {} exited with {}", label, s); false }
        Err(e) => { eprintln!("Auto-SCIP: failed to run {}: {}", label, e); false }
    }
}

fn try_lsp_bridge(root: &PathBuf, lsp_server: &str, lang: &str) -> bool {
    if !on_path("lsp-to-scip") || !on_path(lsp_server) {
        return false;
    }
    println!("Auto-SCIP: lsp-to-scip + {} — enriching graph...", lsp_server);
    let scip_out = root.join("index.scip");
    match std::process::Command::new("lsp-to-scip")
        .args(["--server", lsp_server, "--lang", lang, "--out", "index.scip"])
        .current_dir(root)
        .status()
    {
        Ok(s) if s.success() && scip_out.exists() => true,
        Ok(s) => { eprintln!("Auto-SCIP: lsp-to-scip exited with {}", s); false }
        Err(e) => { eprintln!("Auto-SCIP: lsp-to-scip failed: {}", e); false }
    }
}

fn import_scip_and_cleanup(root: &PathBuf) {
    let scip_out = root.join("index.scip");
...
fn auto_scip(root: &PathBuf, result: &infigraph_core::IndexResult) -> Result<()> {
    ...
    if lang_counts.is_empty() { return Ok(()); }
    let dominant = lang_counts.iter().max_by_key(|(_, c)| *c).map(|(l, _)| l.clone()).unwrap();
    ...
    for (lang_tags, scip_cmd, scip_args, lsp_server, lsp_lang, install_hint) in entries {
        if !lang_tags.iter().any(|t| *t == dominant) { continue; }
...
fn cmd_languages(project_root: Option<&Path>) -> Result<()> {
    ...
    for pack in registry.languages() {
        ...
        println!("  {} ({}) [{}]", pack.name, pack.extensions.join(", "), backend);
    }
    Ok(())
}

fn cmd_symbols(root: &PathBuf, file: &str) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_query(root: &PathBuf, cypher: &str) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_dead_code(root: &PathBuf) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_impact(root: &PathBuf, symbol: &str, depth: u32) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_group(root: &PathBuf, action: GroupAction) -> Result<()> {
    use infigraph_core::multi::Registry;
    ...
    match action {
        ...
        GroupAction::Index { group, full } => {
            ...
            for repo_name in &g.repos {
                ...
                if full {
                    ...
                    if tg_dir.exists() {
                        ...
                        if had { let _ = std::fs::rename(&sess_dir, &sess_bak); }
                        std::fs::remove_dir_all(&tg_dir)?;
                        if had {
                            std::fs::create_dir_all(&tg_dir)?;
            ...
            println!("\nDone. All {} repos in group '{}' indexed.", g.repos.len(), group);
        }
        GroupAction::Sync { group } => {
            let count = infigraph_core::multi::sync_group_contracts(
                &mut registry, &group, bundled_registry,
            )?;
        ...
        GroupAction::Contracts { group } => {
            ...
            if g.contracts.is_empty() {
                println!("No contracts discovered in group '{}'. Run 'infigraph group sync {}' first.", group, group);
            } else {
                println!("Contracts in group '{}':", group);
                for c in &g.contracts {
                    println!("  {} {:>4} {:30} ({}) {}", c.service, c.method, c.path, c.symbol_id, c.file);
                }
            }
        }
        GroupAction::Deps { group } => {
            let deps = infigraph_core::multi::detect_cross_service_deps(
                &registry, &group, bundled_registry,
            )?;
            if deps.is_empty() {
                ...
                for d in &deps {
                    println!("  {} ({}) → {} {} {} [{}]",
                        d.caller_service, d.caller_symbol,
                        d.target_service, d.target_method, d.target_path,
                        d.caller_file);
                }
        ...
        GroupAction::Link { group } => {
            ...
            println!("Linked {} cross-service CALLS_SERVICE edges in group '{}'.", count, group);
        }
...
fn cmd_repos() -> Result<()> {
    ...
    if registry.repos.is_empty() {
        println!("No repositories registered. Use 'infigraph group add <group> <repo>' to register.");
        return Ok(());
    }
...
fn find_mcp_binary() -> Result<PathBuf> {
    let bin_name = if cfg!(windows) { "infigraph-mcp.exe" } else { "infigraph-mcp" };
    ...
    let lookup = if cfg!(windows) { "where" } else { "which" };
    if let Ok(output) = std::process::Command::new(lookup)
        .arg(bin_name)
        .output()
    {
...
"#);
    ...
    println!("  Updated primary search instructions in {}", claude_md.display());
...
"#;
    if !reindex_cmd.exists() {
        ...
        println!("  Added /infigraph-reindex command to {}", reindex_cmd.display());
    } else {
        println!("  /infigraph-reindex command already exists at {}", reindex_cmd.display());
    }
...
fn install_enforcement_hook(home: &std::path::Path) -> Result<()> {
    ...
    if !already_exists {
        ...
        println!("  PreToolUse hook already configured in {}", settings_path.display());
    }
...
fn install_session_save_hook(home: &std::path::Path) -> Result<()> {
    ...
    println!("  Installed session save hook: {}", save_hook_path.display());
    println!("  Installed session reset hook: {}", reset_hook_path.display());
    println!("  Installed session start hook: {}", start_hook_path.display());
    ...
    if !save_exists {
        ...
        println!("  Added UserPromptSubmit hook to {}", settings_path.display());
    } else {
    ...
    if !reset_exists {
        ...
        settings_changed = true;
        println!("  Added PostToolUse reset hook to {}", settings_path.display());
    } else {
...
fn install_models(mcp_path: &std::path::Path, home: &std::path::Path) -> Result<()> {
    let dest = home.join(".infigraph").join("models").join("potion-base-8M");
    ...
    let src_size = std::fs::metadata(src.join("model.safetensors")).map(|m| m.len()).unwrap_or(0);
    let dest_size = std::fs::metadata(dest.join("model.safetensors")).map(|m| m.len()).unwrap_or(0);
...
fn cmd_uninstall() -> Result<()> {
    ...
    if claude_md.exists() {
        ...
        if let Some(start) = content.find(marker) {
            ...
            std::fs::write(&claude_md, if new_content.is_empty() { String::new() } else { format!("{}\n", new_content) })?;
            println!("  Removed primary search instructions from {}", claude_md.display());
        }
    }
    ...
    let reindex_cmd = home.join(".claude").join("commands").join("infigraph-reindex.md");
    if reindex_cmd.exists() {
    ...
    if settings_path.is_file() {
        ...
        if let Ok(mut settings) = serde_json::from_str::<serde_json::Value>(&content) {
            ...
            for event in &["PreToolUse", "UserPromptSubmit", "PostToolUse", "SessionStart"] {
                if let Some(arr) = settings["hooks"].get_mut(*event).and_then(|v| v.as_array_mut()) {
                    let before = arr.len();
                    arr.retain(|entry| !infigraph_hook(entry));
                    if arr.len() < before {
                        println!("  Removed {} hook(s) from {}", event, settings_path.display());
                    }
                }
            }
            let pretty = serde_json::to_string_pretty(&settings)?;
...
fn cmd_update() -> Result<()> {
    ...
    let script_url = if is_ghe {
        format!("https://{}/api/v3/repos/{}/{}/contents/install.sh", gh_host, gh_owner, gh_repo)
    } else {
        format!("https://raw.githubusercontent.com/{}/{}/main/install.sh", gh_owner, gh_repo)
    };

    let cmd = if is_ghe {
        format!("gh api -H 'Accept: application/vnd.github.raw' --hostname {} '{}' | bash", gh_host, script_url)
    } else {
...
fn cmd_search(root: &PathBuf, query: &str, limit: usize, alpha: f32) -> Result<()> {
...
fn cmd_snippet(root: &PathBuf, symbol_id: &str) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    ...
    let snippet =
        infigraph_core::search::read_lines_from_file(&file_path, detail.start_line, detail.end_line)?;
...
fn cmd_architecture(root: &PathBuf) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_cluster(root: &PathBuf) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_export(root: &PathBuf, format: &str, output: Option<PathBuf>) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_detect_changes(root: &PathBuf, base: &str, depth: u32) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_visualize(root: &PathBuf) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_routes(root: &PathBuf) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_visualize_symbol(root: &PathBuf, symbol_id: &str, depth: u32) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_watch(root: &PathBuf, debounce: u64) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;

    println!("Watching {} (debounce {}ms) — Ctrl-C to stop", root.display(), debounce);
...
fn cmd_scip_import(root: &PathBuf, index_path: &PathBuf) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
...
fn cmd_index_manifests(root: &PathBuf) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    let store = prism.store().context("graph not initialized")?;
    ...
    println!("Indexed {} manifests, {} dependencies:\n", results.len(), total);
    for r in &results {
        println!("  {} [{}]: {} deps", r.manifest_file, r.ecosystem, r.deps.len());
    }
    Ok(())
}

fn cmd_dependencies(root: &PathBuf, ecosystem: Option<&str>) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    let store = prism.store().context("graph not initialized")?;
...
fn cmd_find_refs(root: &PathBuf, symbol: &str) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    let store = prism.store().context("graph not initialized")?;
...
fn cmd_api_surface(root: &PathBuf, file_filter: Option<&str>) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    let store = prism.store().context("graph not initialized")?;
...
fn cmd_file_deps(root: &PathBuf, file: &str) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    let store = prism.store().context("graph not initialized")?;
    ...
    for f in &deps.imports { println!("    → {}", f); }
    if deps.imports.is_empty() { println!("    (none)"); }
    println!("\n  Imported by ({}):", deps.imported_by.len());
    for f in &deps.imported_by { println!("    ← {}", f); }
    if deps.imported_by.is_empty() { println!("    (none)"); }
    Ok(())
}

fn cmd_type_hierarchy(root: &PathBuf, symbol: &str, depth: u32) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    let store = prism.store().context("graph not initialized")?;
    ...
    for a in &hier.ancestors { println!("    ↑ {} [{}]  ({})", a.name, a.kind, a.file); }
    if hier.ancestors.is_empty() { println!("    (none — root type)"); }
    println!("\n  Descendants ({}):", hier.descendants.len());
    for d in &hier.descendants { println!("    ↓ {} [{}]  ({})", d.name, d.kind, d.file); }
    if hier.descendants.is_empty() { println!("    (none — leaf type)"); }
    Ok(())
}

fn cmd_test_coverage(root: &PathBuf, file_filter: Option<&str>) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    let store = prism.store().context("graph not initialized")?;
    ...
    println!("Test Coverage: {}%  ({} covered / {} uncovered)\n",
...
fn cmd_security(root: &PathBuf, severity: Option<&str>, category: Option<&str>) -> Result<()> {
    ...
    if let Some(sev) = severity {
        ...
        scan.findings.retain(|f| f.severity.to_string() == sev_upper);
    }
    if let Some(cat) = category {
        let cat_norm = cat.to_lowercase().replace(' ', "");
        scan.findings.retain(|f| f.category.to_string().to_lowercase().replace(' ', "") == cat_norm);
    }
...
fn cmd_complexity(root: &PathBuf, threshold: u32, file: Option<&str>) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    ...
    println!("Complexity: {} symbols, avg {:.1}, {} hotspots (>= {})\n",
        rows.len(), avg, hotspots.len(), threshold);
...
fn cmd_refactor(root: &PathBuf, target: Option<&str>, focus: &str, limit: usize) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    ...
    let focus = infigraph_core::refactor::Focus::from_str(focus);
    let recs = infigraph_core::refactor::analyze(&conn, emb_ref, target, focus, limit)?;
    print!("{}", infigraph_core::refactor::format_recommendations(&recs, target));
    Ok(())
}

fn cmd_semantic_diff(root: &PathBuf, old_ref: &str, new_ref: &str) -> Result<()> {
    let canonical = root.canonicalize().context("invalid project root")?;
    let registry = bundled_registry()?;
...
fn cmd_sequence(root: &PathBuf, symbol_id: &str, depth: u32) -> Result<()> {
    let registry = bundled_registry()?;
    let mut prism = Infigraph::open(root, registry)?;
    prism.init()?;
    let store = prism.store().context("not initialized")?;
