        ...
        if remote {
            ...
            match result {
                infigraph_core::daemon_protocol::WriteResult::Ok {
                    ...
                    // The daemon rebuilds symbols, calls and TESTED_BY
                    // edges, but not SCIP enrichment -- that's a separate
                    // external-indexer step this process would otherwise
                    // spawn after a local index. Since the rebuild replaced
                    // the graph wholesale, any enrichment it held is gone,
                    // and the next ordinary `infigraph index` won't restore
                    // it either (every file hash matches after a full
                    // rebuild, so its analyzer steps are skipped).
                    eprintln!(
                        "warning: SCIP enrichment (compiler-grade types and cross-file \
                         references) was not regenerated by the daemon's rebuild. If this \
                         project relies on it, re-import with `infigraph scip-import`, or \
                         stop the daemon and run `infigraph index --full` locally."
                    );
                }
