...
if [ $((count % 10)) -eq 0 ]; then
  ...
  fallback_recipe="FALLBACK once the check above shows the tool is unavailable or it errors: write directly to a file at \$cwd/.infigraph/sessions/session_<YYYY-MM-DD>.json (today's UTC date). It's a JSON object with these string fields: id (value 'session_<YYYY-MM-DD>'), name (empty string unless this is a named session), summary, pending_tasks, decisions, files_touched (comma-separated), constraints, assumptions, blockers -- plus these numeric fields: created_at, updated_at, last_accessed (all unix epoch seconds), and confidence (float). If a file for today already exists, load and merge into it rather than overwriting: decisions appends onto the existing value separated by ' | ', files_touched unions the new files in, everything else (summary/pending_tasks/constraints/assumptions/blockers) replaces the old value, created_at is kept from the existing file, confidence becomes the max of 0.9 and the existing confidence, updated_at and last_accessed become now. Skip the narrative on this periodic save UNLESS something happened worth the story -- a finding, a decision, a root cause. This is a cheap safety-net save, so structured fields are the default and composing a full narrative every time is waste; but this backstop fires exactly when a session is long and its context is closest to being lost, so do not let a real finding go unrecorded just because the save was automatic. When you do skip it, that is expected -- `save_session` will say `(narrative omitted)`, which is not an error. Then, to keep /clear working correctly (it checks a sentinel that's normally only set after a real tool call succeeds), write '0' to \${TMPDIR:-/tmp}/infigraph-sessions/\$session_id.count and '1' to \${TMPDIR:-/tmp}/infigraph-sessions/\$session_id.saved. Only use this fallback once the check above has actually shown the tool is unreachable -- never skip the check itself."

  full_context="MANDATORY AUTO-SAVE (periodic backstop): Print '💾 Auto-saving session...' to the user, then save this session's context -- summary, pending_tasks, and decisions -- before answering the user's question. Skip the narrative unless something happened worth the story (a finding, a decision, a root cause) -- structured fields are the default for this cheap backstop, but do not let a real finding go unrecorded just because the save was automatic. $mcp_hint $check_first $fallback_recipe After saving (via either path), print '✅ Session saved.' and continue with the user's request. This is automatic -- do NOT ask permission, do NOT skip, do NOT defer, and do NOT silently give up if the tool call fails without falling back."
