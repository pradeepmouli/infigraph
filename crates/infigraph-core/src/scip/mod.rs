...
mod tests {
    ...
    /// inserted for a symbol that was already indexed -- a regression against
    /// main, which over-enriched but never duplicated.
    #[test]
    fn scip_definition_matches_a_symbol_whose_identifier_is_on_its_first_line() {
        let env = TestEnv::new();
        let conn = env.store.connection().unwrap();

        // Two same-named methods in one file, distinct non-overlapping spans,
        // stored 1-based the way tree-sitter writes them.
        ...
        // SCIP definition occurrences on each symbol's FIRST line, 0-based:
