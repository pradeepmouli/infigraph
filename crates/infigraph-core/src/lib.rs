            ...
            _ => match open_kuzu_with_retry(
                ...
                Err(first_err) => {
                    ...
                    if let Some(kb) = recovered {
                        ...
                        {
                            if self.db_path.exists() {
                                use crate::quarantine::WalRecovery;
                                match crate::quarantine::try_recover_by_setting_wal_aside(
                                    parent,
                                    ...
                                    // The graph opens as it stands, in a fresh
                                    // process. Every error-string
                                    // classification has already had its say --
                                    // the retry loop returned early on lock
                                    // contention and on a storage-version
                                    // mismatch, and the WAL-aside attempt has
                                    // ruled on a torn tail. What is left is a
                                    // corruption verdict derived entirely from
                                    // an error THIS process happened to see,
                                    // against an image that demonstrably works.
                                    // Refuse rather than destroy it.
                                    WalRecovery::AlreadyHealthy => {
                                        return Err(last_err).context(format!(
                                            "the graph at {} did not open here, but a fresh \
                                             probe process opened AND scanned it successfully \
                                             -- it is not corrupt. Something transient (most \
                                             likely a concurrent writer mid-checkpoint) \
                                             outlasted the retry budget. Refusing to quarantine \
                                             a healthy graph; retry the operation.",
                                            self.db_path.display()
                                        ));
                                    }
