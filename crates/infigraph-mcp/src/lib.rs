...
pub const MCP_TO_CLI_MAP: &[(&str, &str)] = &[
    ...
    ("enable_watch", "watch enable"),
    ("disable_watch", "watch disable"),
    ("restart_watch", "watch restart"),
    ("enable_watch_docs", "watch-docs enable"),
    ("disable_watch_docs", "watch-docs disable"),
    ("restart_watch_docs", "watch-docs restart"),
...
pub fn dispatch_tool(tool_name: &str, args: &Value) -> Result<String, anyhow::Error> {
    match tool_name {
        ...
        "enable_watch" => tools::watch::enable_watch(args),
        "disable_watch" => tools::watch::disable_watch(args),
        "restart_watch" => tools::watch::restart_watch(args),
        ...
        "enable_watch_docs" => tools::docs::enable_watch_docs(args),
        "disable_watch_docs" => tools::docs::disable_watch_docs(args),
        "restart_watch_docs" => tools::docs::restart_watch_docs(args),
...
pub fn build_tools_list() -> Vec<Value> {
    vec![
        ...
        tool_def("search", "PRIMARY: Unified search — finds symbols by name, meaning, or text pattern in one call. Runs keyword-hybrid (BM25+vector) AND semantic-hybrid AND a text search together: ranked symbols, plus every line containing the query under 'Text matches', each naming its enclosing symbol (symbols holding a match rank first). To enumerate every occurrence of a string or pattern (e.g. all call sites before a rename), set regex=true: all matching lines are listed, not just `limit`. Auto-escalates internally when results are weak — no need to retry with different tools. Use this INSTEAD OF grep/ripgrep/find for ALL search. Set scope='docs' for document-only search.",
            p(true,false,false,json!({"query":{"type":"string","description":"Search query (symbol name, natural language, or text pattern)"},"limit":{"type":"integer","default":20},"kind":{"type":"string","description":"Optional: filter by symbol kind (Function, Method, Class, etc.)"},"file_pattern":{"type":"string","description":"Optional: glob to restrict text search (e.g. '*.py')"},"scope":{"type":"string","enum":["code","docs","all"],"default":"all","description":"Search scope: code (symbols and text matches), docs (documents only), all (both)"},"regex":{"type":"boolean","default":false,"description":"If true, treat query as a raw regex (not escaped), rank the symbols containing a match first, and list every matching line rather than the first `limit` -- the way to enumerate all occurrences"},"detail":{"type":"boolean","default":false,"description":"If true, return full source snippets and doc excerpts. Default (false) returns compact one-line-per-result format."}})), &["path","query"]),
        ...
        tool_def("trace_callers", "PRIMARY: Find all direct callers of a symbol. Use INSTEAD OF grep for 'who calls this function'. Returns caller symbol IDs, files, and line numbers. Set include_tests=false to exclude callers that are test functions/methods (useful when auditing production call paths). WARNING: if symbol_id is one method of a multi-method class/interface, callers of SIBLING methods on the same class are NOT included by default — the response will say so when this applies. Set expand_interface=true to aggregate across every sibling method when you actually want the interface-wide blast radius (e.g. before changing the interface itself, not just this one method). The symbol_id is a two-step lookup: first find it via `search` or `search_symbols` (results include the exact id, e.g. 'app/auth.py::login'), then pass that id here.",
            p(true,true,false,json!({"include_tests":{"type":"boolean","default":true,"description":"Include test-function/method callers. Set false to see only non-test callers."},"expand_interface":{"type":"boolean","default":false,"description":"Aggregate callers across every sibling method on the same class/interface, not just symbol_id. Use when assessing the impact of changing the whole interface."}})), &["path","symbol_id"]),
        ...
        tool_def("transitive_impact", "PRIMARY: Find all symbols transitively affected by changes to a symbol. Use BEFORE any refactor to understand blast radius. Follows CALLS edges in reverse. WARNING: if symbol_id is one method of a multi-method class/interface, this only traces callers of THAT method — callers reaching the interface through a sibling method are NOT included, and the response will say so when this applies. Set expand_interface=true for the true interface-wide blast radius (e.g. before changing the interface's contract, not just one method's implementation).",
            p(true,true,false,json!({"depth":{"type":"integer","default":5},"expand_interface":{"type":"boolean","default":false,"description":"Aggregate impact across every sibling method on the same class/interface, not just symbol_id. Use when assessing the impact of changing the whole interface."}})), &["path","symbol_id"]),
        ...
        tool_def("enable_watch", "Enable code-watching for a project: persists the enabled policy to .infigraph/config.toml and starts a watcher if none is running. Works whether a daemon is running or not.",
            p(true,false,false,json!({})), &["path"]),
        tool_def("disable_watch", "Disable code-watching for a project: persists the disabled policy to .infigraph/config.toml and stops the running watcher, if any. Works whether a daemon is running or not.",
            p(true,false,false,json!({})), &["path"]),
        tool_def("restart_watch", "Restart code-watching for a project without changing the enabled/disabled policy.",
            p(true,false,false,json!({})), &["path"]),
        ...
        tool_def("enable_watch_docs", "Enable doc-watching for a project: persists the enabled policy to .infigraph/config.toml and starts a doc watcher if none is running. Works whether a daemon is running or not.",
            p(true,false,false,json!({})), &["path"]),
        tool_def("disable_watch_docs", "Disable doc-watching for a project: persists the disabled policy to .infigraph/config.toml and stops the running doc watcher, if any. Works whether a daemon is running or not.",
            p(true,false,false,json!({})), &["path"]),
        tool_def("restart_watch_docs", "Restart doc-watching for a project without changing the enabled/disabled policy.",
            p(true,false,false,json!({})), &["path"]),
