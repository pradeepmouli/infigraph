...
- `cargo build --workspace` then `cargo test --workspace --no-fail-fast` on the merged branch (this repo's CI equivalent — see root AGENTS.md). Run test in background (`run_in_background: true`) if it's a large workspace; check `.output` file or wait for the completion notification rather than polling.
