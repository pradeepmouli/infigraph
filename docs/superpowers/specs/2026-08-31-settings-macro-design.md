...
**`registry`** — done (`docs/superpowers/plans/2026-09-01-remaining-settings-migration.md`), split by owning module into two groups. **`install`** (`crates/infigraph-core/src/lib.rs`, `install_settings()`): `INFIGRAPH_INSTALL_DIR` (already fit); `INFIGRAPH_BIN`, `INFIGRAPH_GH_HOST`, `INFIGRAPH_GH_OWNER` (upstream-inherited, seeded from the legacy names; canonical `INFIGRAPH_INSTALL_{BIN,GH_HOST,GH_OWNER}` also work). Collapsed `cli/install.rs`'s three duplicate `GH_HOST`/`GH_OWNER` read sites onto the one accessor. **`registry`** (`crates/infigraph-core/src/instances.rs`, `registry_settings()` — declared there, not in `multi`, because `multi::Registry` would collide with the generated struct): `INFIGRAPH_REGISTRY_HOME` (already fit); `INFIGRAPH_INSTANCES_DIR` → `INFIGRAPH_REGISTRY_INSTANCES_DIR` (fork-only, renamed); `INFIGRAPH_ORG` (inherited, shimmed; canonical `INFIGRAPH_REGISTRY_ORG`).

**`llm`** — done (`crates/infigraph-core/src/review/llm.rs`, `llm_settings()`): `INFIGRAPH_LLM_{MODEL,BASE_URL,MAX_TOKENS,EXTRACT}` all already fit the convention, no shims; `EXTRACT` became a `Toggle` ("0"/"false" now off — previously any value enabled it). Correction: `INFIGRAPH_API_KEY` is not an LLM setting — it is the MCP HTTP-transport bearer key (`mcp/web/mod.rs::check_auth`), now its own **`web`** group (`crates/infigraph-mcp/src/web/mod.rs`; inherited, shimmed; canonical `INFIGRAPH_WEB_API_KEY`; empty disables auth).
...
3. All remaining groups are migrated (`registry`/`install`, `llm`/`web`, `session`/`embed` — `docs/superpowers/plans/2026-09-01-remaining-settings-migration.md`; inherited names seeded via the one-line `settings::legacy_env` helper). `graph` — done (`docs/superpowers/plans/2026-09-01-graph-settings-migration.md`); it also settled the inherited-var shim strategy: read the legacy name, seed the CLI slot (the `INFIGRAPH_BACKEND` precedent), never a second mechanism. `watch` — done (`docs/superpowers/plans/2026-09-01-watch-settings-migration.md`); its 5 real fields were all fork-only (confirmed via upstream content search) and got renamed to a consistent `INFIGRAPH_WATCH_*` prefix rather than shimmed, and two inconsistent boolean-truthy conventions were unified via a new reusable `Toggle` field type. `INFIGRAPH_WATCH_DAEMON` turned out to be dead code (removed from this bullet); `INFIGRAPH_WATCH_ENABLED`/`INFIGRAPH_WATCH_DOCS_ENABLED`/`INFIGRAPH_NO_WATCH`/`INFIGRAPH_TEST_DAEMON_PANIC` stay unmigrated (documented reasons in that plan's Global Constraints).
