...
- **R2.3.7 — Per-project write lock (the actual fix for I-4).** Every worker process must acquire `.infigraph/graph.lock` (R2.1.1, per-project, independent of the global `mcp.lock`) before any graph write, and hold it only for the write's duration — not for the worker's lifetime, since a worker legitimately serves many projects across its life. **Partially landed, gap identified 2026-07-31:** `graph.lock`'s per-call leasing exists (`docs/superpowers/specs/2026-07-20-write-safety-locks-design.md`, PR 1-2), but `GraphStore::open_with_lock_timeout` still calls `Database::new()` *before* ever acquiring `graph.lock` (`store.rs:123` then `:126`) — the lock only gates schema-init on an already-open `Database`, never the open attempt itself. Two concurrent writers therefore still race at Kuzu's own internal exclusivity check (inside `ffi::new_database`), opaque and undiagnosed, entirely bypassing `graph.lock`. Worse, the watcher's persistent connection (PR #43) blocks every other writer's `Database::new()` for as long as it stays open, regardless of `graph.lock` state, since Kuzu's check is keyed on "is a handle open," not on lock state — this is upstream issue #46. The concrete fix (lock-before-open ordering + a watcher connection-yield mechanism, using `watch.lock`-gated contention detection) is fully specced in `docs/superpowers/specs/2026-07-31-graph-lock-write-coordination-design.md`, pending implementation. Until it lands, two concurrent sessions independently calling `index_project` (or the CLI calling `infigraph index` while any MCP worker or the watcher is also live) on the same project can still hit Kuzu's raw, undiagnosed failure rather than infigraph's own bounded-wait/`Busy` handling.
