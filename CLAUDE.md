...
- `cargo test -p infigraph-mcp` (notably `--test groups_watch_perf`, which the pre-commit hook runs) does **not** rebuild the separate `infigraph-cli` binary target, yet the test's auto-watch path spawns it from the fixed path `.shared-target/debug/infigraph`. After switching between branches with different `lbug` (Kuzu) versions, that stale binary fails silently ("No results across repos" — the version mismatch only shows in the spawned daemon's log). Run `cargo build -p infigraph-cli` after a branch switch before trusting that test.
- `write_lock_perf::test_contended_lock_throughput` (pre-commit hook) asserts 4-thread contended throughput `< 8×` single-thread; on this machine under ordinary load (~6–7, an editor at 100%+ CPU) it lands at 8–11× and flaked 2 in 3 hook runs all day with numbers identical before and after unrelated changes. Retry the commit; treat it as a regression only if the numbers move well beyond that range.
- `compression_eval.rs::phase3_dedup_eval` persists `.infigraph/dedup_state.json` into `crates/infigraph-mcp/` (its cwd) on a passing run, so the *next* run sees the placeholder on the first call and fails the `>= 50 tokens` gate. Delete that file before re-running (tracked in #136).
