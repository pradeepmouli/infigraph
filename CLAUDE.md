...
- **Reads do not open the stores outside the daemon.** Under `INFIGRAPH_BACKEND=daemon`, `DaemonKuzuBackend` (code graph) and `DaemonDocStore` (documents) route every read over a local socket to the daemon's read service; `INFIGRAPH_DIRECT_READS=1` is the sole exception, and it is deliberately explicit rather than an automatic fallback — a silent fallback keeps both paths permanently live, which is how `Infigraph::init` and `GraphStore::open_read_only_or_degrade` drifted until one quarantined healthy graphs (5818aa1). A new read belongs on `GraphQuery`/`DocQuery`, which are generic over `QueryExec`, so one query body serves both the local and routed paths; adding it to a backend instead forces a second copy for the routed path.
